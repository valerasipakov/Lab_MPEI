...
файл %s\n", argv[1]);
	...
	while (nr) {
		...
		if (nr > 0)
			if (toy.price < maxPrice && (toy.minAge <= 5)) {
...
void CorrectFile(int argc, char* argv[]) {
	...
	while (nr == 1 && nw == 1) {
		...
		if (toy.maxAge - toy.minAge < 2 && toy.maxAge + 2 <= 150) { toy.maxAge = toy.maxAge + 2; flag = 1; }
		if (toy.maxAge - toy.minAge < 2 && toy.maxAge + 2 > 150) { toy.minAge = toy.minAge - 4; flag = 1; }
...
void ViewFile(int argc, char* argv[]) { //Выводим на экран все записи
	...
	printf("Просмотр:\n");
	while (nr == 1) {
		printf("|         Цена          |          Мин возраст        |      Макс возраст      |      Название              \n");
		printf("|%23.d|%29.d|%24.d|      %s", toy.price, toy.minAge, toy.maxAge, toy.name);
		printf("\n");
		nr = fread(&toy, sizeof(toy), 1, fb);
	}
	fclose(fb);
